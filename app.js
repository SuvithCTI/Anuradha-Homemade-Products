...
function setupEventListeners() {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', generateWhatsAppOrder);
  }
...
// Helper to build floating raw ingredient emojis for the X-Ray packaging viewer
function getXRayOverlay(id) {
  const ingredientsMap = {
    'amla-powder': {
      items: ['🍈', '🍃', '✨', '🍈', '🍃'],
      label: '100% Sun-Dried Amla Only'
    },
    'sweet-amla-candy': {
      items: ['🍈', '🍬', '🍋', '🍈', '✨'],
      label: 'Wild Amla & Organic Sugar'
    },
    'nuts-powder': {
      items: ['🌰', '🥜', '🎃', '🌱', '🌰', '🥜'],
      label: 'Sprouted Nuts & Seeds'
    },
    'ragi-almond-powder': {
      items: ['🌾', '🥜', '✨', '🌾', '🥜'],
      label: 'Sprouted Ragi & Almonds'
    },
    'healthy-mix': {
      items: ['🌾', '🌽', '🥜', '🌰', '🫘', '🌱'],
      label: '18 Sprouted Grains & Nuts'
    },
    'millet-health-mix': {
      items: ['🌾', '🌽', '🌱', '🌾', '🌱'],
      label: '9 Premium Organic Millets'
    },
    'cow-ghee': {
      items: ['🧈', '🥛', '🔥', '✨', '🧈', '✨'],
      label: 'Vedic Bilona Churned Ghee'
    },
    'forest-honey': {
      items: ['🐝', '🍯', '🌸', '🌿', '🍯', '🐝'],
      label: '100% Unprocessed Wild Honey'
    }
  };

  const data = ingredientsMap[id];
  if (!data) return '';

  let particlesHtml = '';
  data.items.forEach((item, idx) => {
    const driftY = (Math.random() * 60 - 30) + 'px';
    ...
    particlesHtml += `
      <span class="xray-particle" style="
        top: ${top}%;
        left: ${left}%;
  ...
  return `
    <div class="product-xray-view">
      ${particlesHtml}
      <span class="xray-label">${data.label}</span>
    </div>
  `;
}
...
function initAlchemyMixer() {
  ...
  ingredients.forEach(ing => {
    ...
    // Touch support fallback for tablets & mobiles
